...
\begin{itemize}
    ...
    \item $\mathbf{w}$ is a weight vector, tilting the line
    \item $\mathbf{x}$ is a data point
    \item $b$ is a bias, lifting the line up along the $y$-axis
\end{itemize}

$\mathbf{w}$ is such that if for an $\mathbf{x}$ observation, if $y$ is
positive, then the observation belongs to one category (the $\blacksquare$
category). If $y$ is negative, the observation belongs to the $\circ$
category.
}

\pause

In SVM we want a margin $M$, so we can rewrite the linear
classifier.
...
Now suppose that we pick a point $\mathbf{x}^\blacksquare$ that lies on the $\blacksquare$ class
boundary line, \ie $\mathbf{w}^T\mathbf{x}^\blacksquare = M$.
    
$\mathbf{x}^\blacksquare$ is a \textbf{support vector}.
...
We are using some function $\theta(x_i)$ from input $x_i$.
...
\item Example, separating odd from even numbers
\end{itemize}
...
\item Starting from input data $x_1, x_2, \dots, x_d$,
  square all data $x^2_1, x^2_2, \dots, x^2_d$,
  and also take all products of the
        data $x_1 x_2, x_1 x_3, \dots, x_{d−1} x_d$.

\item We've transformed a data set of $d$ elements into a set with
    $\frac{d^2}{2}$ elements.
\end{itemize}
...
For example, for one dimension $x_1$ the data is linearly
    inseparable. But adding a second dimension $x^2_1$ results in the
